...
let getCategoriesRankingQuery = `SELECT c.name AS category, COUNT(a.category_id) AS count FROM categories AS c
LEFT JOIN articles AS a ON c.id = a.category_id
GROUP BY c.id
HAVING count > 0
ORDER BY count desc;`;
let getCategorizeArticlesQuery = `SELECT A.id, A.title, C.name AS category, A.contents, T.name AS thumbnailName, T.url AS thumbnailUrl, T.full_path AS thumbnailFullPath, S.id AS seriesId, S.name AS series, A.series_order AS seriesOrder, DATE_FORMAT(A.created_at, '%Y-%m-%d %T') AS createdAt, DATE_FORMAT(A.updated_at, '%Y-%m-%d %T') AS updatedAt, DATE_FORMAT(A.deleted_at, '%Y-%m-%d %T') AS deletedAt,  GROUP_CONCAT(tags.name) AS tags FROM articles AS A
JOIN categories AS C ON C.id = A.category_id
JOIN series AS S ON S.id = A.series_id
JOIN thumbnails AS T ON T.id = A.thumbnail_id
JOIN articles_tags AS AT ON AT.article_id = A.id
...
JOIN articles_tags AS AT ON AT.article_id = A.id
JOIN tags ON tags.id = AT.tag_id
WHERE A.title LIKE '%SEARCH_WORD%'
OR C.name LIKE '%SEARCH_WORD%'
OR A.contents LIKE '%SEARCH_WORD%'
OR tags.name LIKE '%SEARCH_WORD%'
NEXT_SEARCH_CRITERIA
AND A.deleted_at is NULL
GROUP BY A.id
ORDER BY A.updated_at desc;`;
...
let getASpecificArticleQuery = `SELECT A.id, A.title, C.name AS category, A.contents, T.name AS thumbnailName, T.url AS thumbnailUrl, T.full_path AS thumbnailFullPath, S.id AS seriesId, S.name AS series, A.series_order AS seriesOrder, DATE_FORMAT(A.created_at, '%Y-%m-%d %T') AS createdAt, DATE_FORMAT(A.updated_at, '%Y-%m-%d %T') AS updatedAt, DATE_FORMAT(A.deleted_at, '%Y-%m-%d %T') AS deletedAt, GROUP_CONCAT(tags.name) AS tags FROM articles AS A
JOIN categories AS C ON C.id = A.category_id
JOIN series AS S ON S.id = A.series_id
JOIN thumbnails AS T ON T.id = A.thumbnail_id
JOIN articles_tags AS AT ON AT.article_id = A.id
